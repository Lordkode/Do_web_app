<template>
  <div class="w-full bg-white rounded-lg border p-6 divide-y">
    <div class="flex place-items-center space-x-2 pt-4 pb-4">
      <div
        class="w-[48px] h-[48px] border border-transparent rounded-full flex place-items-center place-content-center bg-[#fcd4df]"
      >
        <p class="font-black text-[#1D1D1D]">AR</p>
      </div>
      <p>{{ comment.user_id }}</p>
      <hr />
    </div>
    <div class="pt-4 pb-4">
      <div class="flex justify-between">
        <div class="flex place-items-center">
          <startIcon
            v-if="
              parseInt(comment.note) == 1 ||
              parseInt(comment.note) === 2 ||
              parseInt(comment.note) === 3 ||
              parseInt(comment.note) === 4 ||
              parseInt(comment.note) === 5
            "
          />
          <startIcon
            v-if="
          ...
          <startIcon
            ...
            "
          />
          <startIcon v-if="parseInt(comment.note) === 4 || parseInt(comment.note) === 5" />
          <startIcon v-if="parseInt(comment.note) === 5" />
        </div>
        <div>
          <p>Actualisé le {{ comment.created_at.split(' ')[0] }}</p>
        </div>
      </div>
      <div class="pt-10 flex flex-col space-y-4">
        <h3 class="font-bold text-xl">{{ comment.title }}</h3>
        <p>
          {{ comment.description }}
        </p>
      </div>
    </div>
    <div class="pt-4 pb-4 flex space-x-8">
